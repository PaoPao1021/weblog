...
export function PostCollection({ posts, language }: { posts: ContentPost[]; language: Language }) {
  ...
  const empty = language === 'zh' ? '这里还没有记录。' : 'No entries here yet.';
    ...
    return <article className="collection-card" key={post.slug}>
      ...
      <div>
        ...
        <div className="collection-footer">
          <span>{post.readingMinutes} {language === 'zh' ? '分钟阅读' : 'min read'}</span>
          <Link href={`/${language}/posts/${post.slug}`}>{read} <ArrowRight size={12} strokeWidth={2.4} aria-hidden="true" /></Link>
        </div>
      </div>
    </article>;
  })}</div>;
