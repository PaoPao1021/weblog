  ...
  return <main id="main-content" className="hero" tabIndex={-1}>
    <motion.div className="hero-inner" initial={reducedMotion ? false : { y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <div className="hero-apps" aria-label={t("Explore my space")}>
        <motion.button whileHover={reducedMotion ? undefined : { y: -2, scale: 1.02 }} whileTap={reducedMotion ? undefined : { scale: 0.97 }} transition={{ type: 'spring', stiffness: 450, damping: 25 }} className="hero-app primary" onClick={() => navigate('projects')}><Code2 size={19} /><span>{t("Projects")}</span><ArrowUpRight size={16} /></motion.button>
        <motion.button whileHover={reducedMotion ? undefined : { y: -2, scale: 1.02 }} whileTap={reducedMotion ? undefined : { scale: 0.97 }} transition={{ type: 'spring', stiffness: 450, damping: 25 }} className="hero-app" onClick={() => navigate('notes')}><FileText size={18} /><span>{t("Notes")}</span><ArrowUpRight size={16} /></motion.button>
        <motion.button whileHover={reducedMotion ? undefined : { y: -2, scale: 1.02 }} whileTap={reducedMotion ? undefined : { scale: 0.97 }} transition={{ type: 'spring', stiffness: 450, damping: 25 }} className="hero-app" onClick={() => navigate('about')}><UserRound size={18} /><span>{t("About")}</span><ArrowUpRight size={16} /></motion.button>
      </div>
      <div className="hero-links">
        ...
        {siteConfig.email ? <a href={`mailto:${siteConfig.email}`}><Mail size={14} />{t("Email")}<ArrowUpRight size={12} /></a> : <button onClick={() => navigate('about')}><Mail size={14} />{t("Email")}<ArrowUpRight size={12} /></button>}
        {siteConfig.resume ? <a href={assetUrl(siteConfig.resume)} target="_blank" rel="noreferrer noopener"><FileText size={14} />{t("Resume")}<ArrowUpRight size={12} /></a> : <button onClick={() => navigate('about')}><FileText size={14} />{t("Resume")}<ArrowUpRight size={12} /></button>}
      </div>
